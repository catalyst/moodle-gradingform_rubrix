...
/**
 * Exports an Excel spreadsheet of the component grades in a rubrix-graded assignment.
 *
...
$PAGE->set_url('/grade/grading/rubrix/export.php', $params);
...
$data = $DB->get_records_sql("SELECT    grf.id AS grfid, crs.shortname AS course, asg.name AS assignment, gd.name AS rubric,
                                ...
                                JOIN {gradingform_rubrix_criteria} grc ON (grc.definitionid = gd.id)
                                JOIN {gradingform_rubrix_levels} grl ON (grl.criterionid = grc.id)
                                JOIN {grading_instances} gin ON gin.definitionid = gd.id
                                JOIN {assign_grades} ag ON ag.id = gin.itemid
                                JOIN {user} stu ON stu.id = ag.userid
                                JOIN {user} rubm ON rubm.id = gin.raterid
                                JOIN {gradingform_rubrix_fillings} grf ON (grf.instanceid = gin.id)
...
$pos = report_componentgrades_add_header($workbook, $sheet, $course->fullname, $cm->name, 'rubrix', $first->rubric, $showgroups);
